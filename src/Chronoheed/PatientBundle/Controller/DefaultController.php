...
use AppBundle\Form\PatientBillingDetailType;
...
class DefaultController extends Controller
{
    ...
    public function newpatientAction(Request $request)
    {
        ...
        $patient_data['rating'] = 5;//Set rating to 5 by default
        ...
        return $this->createPatient($patient_data);
        ...
        try {
            ...
            if ($form->isValid()) {
                ...
                try {
                    ...
                    $em->persist($patient);
                    $em->flush();
                    return array("patientId" => $patient->getId());
     ...
     * @View
     */
    public function newPatientPhotoAction(Request $request, $patient_id)
    {
        try{

            if( $patient = $this->doesPatientExist( ["id" => $patient_id] ) ) {
                $oldPhoto = "";
                if($patient->getPhoto() != "") $oldPhoto = $patient->getPhoto();

                $image_file = $request->files->get("patientPhotoFileInput");

                //Check if file  exists
                if ((!$image_file instanceof UploadedFile))
                    throw new FileNotFoundException('patientPhotoFileInput');
                ...
                //Crop image
                $imagemanagerResponse = $this->get("liip_imagine.controller")->filterAction($request, $image_file_web_path, 'patient_photo');
            ...
            //Error the patient doesn't exist
            return new JsonResponse([
                'error' => 'not_found', 'error_description' => "Patient does not exist"
            ], 400);
        }catch (\Exception $e){
            return new JsonResponse([
                'error' => 'exception_error', 'error_description' => $e->getMessage()
     ...
     * @View
     */
    public function returnPatientAction($patient_id)
    {
        if($patient = $this->doesPatientExist(array("id" => $patient_id ) )){
            return $patient;
        }else{
            return new JsonResponse([
     ...
     * @View
     */
    public function updatePatientFileAction($patient_id, Request $request)
    {
        try {
            $req_data = json_decode($request->getContent(), true);
            $patient_data = $req_data['patient'];
            ...
            $patient = $this->doesPatientExist(array("id" => $patient_id));
    ...
    public function returnPatientsAction($q)
    {
        $patients = $this->getDoctrine()->getRepository("AppBundle:Patient")->createQueryBuilder('p')
            ...
            ->orWhere("p.id = :patient_id ")
            ->orWhere("p.identificationnumber like :searched ")
            ->setParameter('searched', "%" . $q . "%")
            ->setParameter('patient_id', $q )
            ->getQuery()
            ->getResult();

//        if (!$patients) {
//            return array("patients" => $patients, "total_patients" => count($patients)) ;//0 results
//        }
    ...
    public function returnPatientProceduresAction($patient_uniqueid)
    {
        $patient_procedures =  $this->getDoctrine()
            ...
            ->select("pp.label", "pp.appointment", "pp.practitionerUniqueid", "pp.postDelay", "pp.cost", "pp.status")
        ...
        if($patient_procedures){
            return array( "recordsTotal" => count($patient_procedures), "data" => $this->formatResultsForDataTable( $patient_procedures ) );
        }
     ...
     * @return array
     */
    private function formatResultsForDataTable($results){
        $array_ = [];
        foreach ($results as $row){
            $rowArr = [];
            foreach ($row as $rowField) {
                array_push($rowArr, $rowField);
            }
    ...
    /**
     * @Get("api/patientsss/{patient_id}/procedures")
     * @param $patient_id
     * @View
     */
    public function returnPatientProceduresssAction($patient_id)
    {
        $patient_procedures =  $this->getDoctrine()
            ->getRepository("AppBundle:PatientProcedure")
            ->createQueryBuilder('pp')
